...
del = this.data.del,
gluecode = this.data.gluecode;
if(!model1 || !modelNumber1 || !productDate1 || !effectiveDate1 || !voucher1 || gluecode1 == ''){
  wx.showToast({
    title: '数据不能为空，添加失败！！！',
    icon: 'none',
    duration: 2000
 ...
 delelement: function (e) {
  ...
  const delid = e.currentTarget.dataset.index
    ...
    that.data.gluecode.splice(delid, 1)
  that.setData({
 ...
 formSubmit:  util.throttle(function (ev) {
  ...
  gluecode = that.data.gluecode;
  console.log(model, modelNumber, productDate, effectiveDate, voucher, gluecode)
  if (model.length != 0 & modelNumber.length != 0 & productDate.length != 0 & effectiveDate.length != 0 & voucher.length != 0 & gluecode.length != 0) {
   wx.request({
     url: 'https://www.liuke.xyz/gluelog/login',
     data:{
         code:'inventory',
         model:JSON.stringify(that.data.model),
         modelNumber:JSON.stringify(that.data.modelNumber),
         ...
         gluecode:JSON.stringify(that.data.gluecode),
       },
         method:'POST',
